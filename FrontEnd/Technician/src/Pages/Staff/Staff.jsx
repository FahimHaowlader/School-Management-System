...
const StaffRow = ({ status, name, emailAddress, staffId, assignedZones, division, imageUrl }) => {
  const badgeStyles = {
    "On Duty": "bg-emerald-50 text-emerald-700 border-emerald-100",
    "Off Duty": "bg-slate-50 text-slate-500 border-slate-200/80",
  ...
  return (
    <tr className="border-b border-slate-200/70 hover:bg-slate-50/70 transition-colors">
      ...
      {/* Custom Entity Modification Controls */}
      <td className="p-4 align-middle text-right">
        <div className="flex items-center justify-end gap-1.5">
          <button 
            onClick={() => console.log("Edit staff parameters")}
          ...
          <button 
            onClick={() => console.log("Remove operational staff assignment")}
...
const Staff = () => {
  const divisions = ["All Divisions", "Administration", "IT Support", "Facilities & Ops", "Security"];
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200/60 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="text-left">
            <h2 className="text-sm font-bold text-slate-900 tracking-tight">Operational Support Staff Directory</h2>
            <p className="text-xs text-slate-400 font-medium mt-0.5">Track system staff infrastructure, manage deployment zones, regulate shift statuses, and store profile logs.</p>
          </div>
          
          <div className="flex items-center gap-4 border-l border-transparent sm:border-slate-100 sm:pl-4">
            <div className="text-center sm:text-right">
              <span className="block text-xl font-black text-slate-900 leading-none">42</span>
              <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Personnel Active</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden transition-all">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-bold text-slate-800 tracking-wide">Onboard New Operational Staff</span>
            </div>
            ...
            <div className="p-5 border-t border-slate-100 space-y-4 bg-white text-left animate-fadeIn">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Duty Status Parameters</label>
                  <select className="w-full h-10 px-3 border border-slate-200 rounded-lg text-xs font-semibold bg-white outline-none focus:border-slate-900 transition-colors">
                    <option>On Duty Shift</option>
                    <option>Off Duty Standby</option>
                    <option>Suspended Assignment</option>
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Assigned Responsibility Zones</label>
                  <input 
                    type="text" 
                    placeholder="e.g., Server Room B, Main Office" 
                ...
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Core Operations Division</label>
                  <select className="w-full h-10 px-3 border border-slate-200 rounded-lg text-xs font-semibold bg-white outline-none focus:border-slate-900 transition-colors">
                    <option>Administration</option>
                    <option>IT Support Network</option>
                    <option>Facilities & Ops</option>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Staff Legal Full Name</label>
                  <input 
                    type="text" 
                ...
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Corporate Email Contact</label>
                  <input 
                    type="email" 
                    placeholder="e.g., r.thorne@centralnet.org" 
                    className="w-full h-10 px-3 border border-slate-200 rounded-lg text-xs outline-none focus:border-slate-900 transition-colors"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Unique Staff Identity String</label>
                  <input 
                    type="text" 
                    placeholder="e.g., STF-2026-441" 
                    className="w-full h-10 px-3 border border-slate-200 rounded-lg text-xs outline-none focus:border-slate-900 transition-colors"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">System Badge Avatar File</label>
                <div className="border border-dashed border-slate-200 rounded-xl p-5 bg-slate-50/50 hover:bg-slate-50 transition-colors flex flex-col items-center justify-center gap-1.5 group/dropzone cursor-pointer relative">
                  ...
                  <p className="text-xs font-bold text-slate-700 mt-0.5">Upload a clean profile verification image file</p>
        ...
        <div className="flex flex-col lg:flex-row items-stretch gap-3">
          <div className="relative flex-grow h-11 group">
            <input 
              type="text" 
              placeholder="Search directory by staff profiles, divisions, system IDs, or deployment zones..." 
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200/80 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                  <th className="p-4">Staff Member Information</th>
                  <th className="p-4">Staff ID</th>
                  <th className="p-4">Deployment Responsibility Zones</th>
                  <th className="p-4 hidden md:table-cell">Operations Division</th>
              ...
              <tbody className="divide-y divide-slate-100">
                <StaffRow 
                  ...
                  assignedZones={["IT Helpdesk", "Server Room Alpha"]}
                  division="IT Support"
                ...
                <StaffRow 
                  status="Off Duty"
                  name="Helena Rostova"
                  emailAddress="h.rostova@centralnet.org"
                  staffId="STF-2026-045"
                  assignedZones={["Admissions Annex", "Records Vault"]}
                ...
                <StaffRow 
                  status="On Duty"
                  name="Arthur Jenkins"
                  emailAddress="a.jenkins@centralnet.org"
                  staffId="STF-2026-089"
                  assignedZones={["Main Entry Gate", "Perimeter Zone 2"]}
                  division="Security"
