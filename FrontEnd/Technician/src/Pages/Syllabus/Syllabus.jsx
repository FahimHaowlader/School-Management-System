...
const SyllabusItem = ({ subject, className, term, teacher, onDownload, onEdit, onDelete }) => (
  <div className="group bg-white border border-slate-100 shadow-sm rounded-xl p-5 transition-all duration-200 hover:shadow-lg cursor-default">
    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
      ...
      {/* Universal Action Controls - Displays all 3 options simultaneously */}
...
const Syllabus = () => {
  ...
  const handleDownload = (subject) => console.log(`Downloading document resource for: ${subject}`);
  const handleEdit = (id) => console.log(`Edit blueprint config ID: ${id}`);
  ...
  const handlePublish = (e) => {
    e.preventDefault();
    if (!formSubject.trim()) return alert("Please specify a subject classification.");
        ...
        {filteredSyllabi.length > 0 ? (
          ...
          <div className="text-center py-12 border border-dashed border-slate-200 rounded-xl bg-white">
            <p className="text-sm font-medium text-slate-400">
              No matching institution records found for this selection scale.
            </p>
          </div>
        )}
      </div>
