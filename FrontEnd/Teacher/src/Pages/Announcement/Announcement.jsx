...
const AnnouncementCard = ({ type, title, description, date, imageUrl, targetAudience }) => {
  const badgeStyles = {
  ...
  return (
    <div className="p-5 bg-white rounded-xl border border-slate-200/70 shadow-sm hover:shadow-lg transition-all group relative text-left">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="w-full md:w-48 lg:w-56 h-40 shrink-0 overflow-hidden rounded-lg border border-slate-100 bg-slate-50">
          <img 
            src={imageUrl} 
            alt={title} 
        ...
        <div className="flex flex-col justify-between flex-1 py-0.5">
          ...
          <div className="flex items-center justify-between ">
            ...
            <div className="flex items-center gap-2">
              {/* REDESIGNED EDIT BUTTON: Standard state is clean/bordered, shifts to solid black background and white text upon user hover */}
              <button 
...
const Announcement = () => {
  const categories = ["All", "Academic", "General", "Urgent"];
  ...
  return (
    <div className="w-full font-sans text-slate-900">
      <div className="space-y-5">
        ...
        {/* Dynamic Context Header Block for Teachers */}
        <div className="bg-white p-4 rounded-xl border border-slate-200/60 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="text-left">
            <h2 className="text-sm font-bold text-slate-900 tracking-tight">Announcement Control Board</h2>
            <p className="text-xs text-slate-400 font-medium mt-0.5">Publish updates, launch dynamic alerts, and manage notice archives.</p>
          </div>
          ...
          <div className="flex items-center gap-4 border-l border-transparent sm:border-slate-100 sm:pl-4">
            <div className="text-center sm:text-right">
              ...
              <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Total Sent</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden transition-all">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-bold text-slate-800 tracking-wide">Compose New Notice Broadcast</span>
            </div>
            ...
            <div className="p-5 border-t border-slate-100 space-y-4 bg-white text-left animate-fadeIn">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Notice Category</label>
                  <select className="w-full h-10 px-3 border border-slate-200 rounded-lg text-xs font-semibold bg-white outline-none focus:border-slate-900 transition-colors">
                    ...
                    <option>Events & Festivities</option>
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Target Classroom Audience</label>
                  <select className="w-full h-10 px-3 border border-slate-200 rounded-lg text-xs font-semibold bg-white outline-none focus:border-slate-900 transition-colors">
                    <option>All Batches (Standard)</option>
                    <option>Class 10 - Section A</option>
                    <option>Class 12 - Science Wing</option>
                    <option>Only Parent Viewports</option>
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Publish Strategy</label>
                  <select className="w-full h-10 px-3 border border-slate-200 rounded-lg text-xs font-semibold bg-white outline-none focus:border-slate-900 transition-colors">
                    <option>Instant Broadcast (SMS + App)</option>
                    <option>Silent Dashboard Feed</option>
                    <option>Save Draft Placeholder</option>
                  </select>
                </div>
              </div>

              <div className="space-y-1">
                <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Headline / Title</label>
                <input 
                  type="text" 
              ...
              <div className="space-y-1">
                <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Notice Description Body</label>
                <textarea 
                  rows={3}
                  placeholder="Provide explicit directions or notes for the cohort regarding this update..." 
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Notice Cover Photo</label>
                <div className="border border-dashed border-slate-200 rounded-xl p-5 bg-slate-50/50 hover:bg-slate-50 transition-colors flex flex-col items-center justify-center gap-1.5 group/dropzone cursor-pointer relative">
                  ...
                  <p className="text-xs font-bold text-slate-700 mt-0.5">Upload a card banner or cover photograph</p>
                  <p className="text-[10px] text-slate-400 font-medium">PNG, JPEG, or WEBP layout up to 5MB</p>
                </div>
              </div>
