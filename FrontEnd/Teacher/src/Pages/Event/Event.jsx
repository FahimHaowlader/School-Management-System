...
const EventCard = ({ category, type, title, description, date, location, imageUrl }) => {
  const badgeStyles = {
    Academic: "bg-emerald-50 text-emerald-700 border-emerald-100",
    Sports: "bg-amber-50 text-amber-700 border-amber-100",
    Cultural: "bg-sky-50 text-sky-700 border-sky-100",
  };
  ...
  return (
    <div className="bg-white border border-slate-200/70 rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all group flex flex-col justify-between text-left">
      <div>
        {/* Cover Thumbnail Image for Visual Context */}
        <div className="w-full h-40 overflow-hidden bg-slate-50 border-b border-slate-100">
          <img 
            src={imageUrl} 
            alt={title} 
        ...
        <div className="p-5">
          <div className="flex gap-1.5 mb-3">
            ...
            <span className="bg-slate-100 border border-slate-200 text-slate-600 text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wide">
              {type}
            </span>
          </div>
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200/60 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="text-left">
            <h2 className="text-sm font-bold text-slate-900 tracking-tight">Event Schedule Control Board</h2>
            <p className="text-xs text-slate-400 font-medium mt-0.5">Program campus calendars, update booking coordinates, and allocate event rosters.</p>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden transition-all">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-bold text-slate-800 tracking-wide">Program New Campus Event</span>
            </div>
            ...
            <div className="p-5 border-t border-slate-100 space-y-4 bg-white text-left animate-fadeIn">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Classification</label>
                ...
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Attendance Scope</label>
                  <select className="w-full h-10 px-3 border border-slate-200 rounded-lg text-xs font-semibold bg-white outline-none focus:border-slate-900 transition-colors">
                    <option>Required Attendance</option>
                    <option>Optional Attendance</option>
                    <option>By Invitation Only</option>
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Target Location Venue</label>
                  <input 
                    type="text"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Event Headline / Title</label>
                  <input 
                    type="text" 
                    placeholder="e.g., Varsity Basketball Playoff Finals" 
                ...
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Date & Time Coordinates</label>
                  <input 
                    type="text" 
                    placeholder="e.g., Nov 05, 2026 | 7:00 PM" 
                    className="w-full h-10 px-3 border border-slate-200 rounded-lg text-xs outline-none focus:border-slate-900 transition-colors"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1">
                <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Event Agenda Description</label>
                <textarea 
                  rows={3}
                  placeholder="Provide precise descriptions, rules, or requirements setup details..." 
              ...
              {/* Combined Media Assets Upload Section (Cover Banner + Document Attachments) */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Left Block: Image Graphic Cover */}
                <div className="space-y-1.5">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Event Cover Photo Banner</label>
                  <div className="border border-dashed border-slate-200 rounded-xl p-4 bg-slate-50/50 hover:bg-slate-50 transition-colors flex flex-col items-center justify-center gap-1 group/cover cursor-pointer relative min-h-[110px]">
                    ...
                    <p className="text-xs font-bold text-slate-700 mt-0.5">Upload a card display display image</p>
                    <p className="text-[10px] text-slate-400 font-medium">PNG or JPEG format up to 5MB</p>
                  </div>
                </div>

                {/* Right Block: File Attachments Section */}
                <div className="space-y-1.5">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Event Material Attachments</label>
                  <div className="border border-dashed border-slate-200 rounded-xl p-4 bg-slate-50/50 hover:bg-slate-50 transition-colors flex flex-col items-center justify-center gap-1 group/attach cursor-pointer relative min-h-[110px]">
                    <input type="file" multiple className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => console.log("Files attached", e.target.files)} />
                    ...
                    <p className="text-xs font-bold text-slate-700 mt-0.5">Attach event schedules or resource guidelines</p>
                    <p className="text-[10px] text-slate-400 font-medium">PDF, DOCX, or Excel sheets up to 10MB</p>
                  </div>
                </div>
              </div>
        ...
        {/* Search Outbox & Filtering Engine Row with Custom Blue Focus States */}
        <div className="flex flex-col lg:flex-row items-stretch gap-3">
          <div className="relative flex-grow h-11 group">
            <input 
              type="text" 
              placeholder="Search programmed event parameters..." 
