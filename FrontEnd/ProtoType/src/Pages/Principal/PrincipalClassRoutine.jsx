...
const PrincipalClassRoutine = () => {
  // Principal-specific states
    ...
    <div className="flex font-sans text-gray-800 ">
      <main className="flex-1 flex flex-col max-w-7xl mx-auto">
        ...
        {/* <div className="mb-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6"> */}
          {/* <div>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">Master Routine</h1>
            <p className="text-slate-500 font-medium mt-1">Reviewing schedule for {selectedClass} Section {selectedSection}</p>
          </div> */}
          ...
          {/* <div className="flex flex-wrap items-center gap-3 bg-white p-2 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <button className="ml-2 p-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all shadow-md shadow-indigo-100">
              <MdPrint size={18} />
            </button>
          </div> */}
        {/* </div> */}
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4 bg-white p-1.5 rounded-xl border border-slate-200">
            ...
            <p className="text-xs font-black text-slate-700 uppercase tracking-widest px-2">Current Week</p>
            <button className="p-1 rounded-lg hover:bg-slate-50 transition-all text-slate-600">
                ...
                {timeSlots.map((slot, index) => (
                  <React.Fragment key={slot}>
                    ...
                    <tr className="group hover:bg-slate-50/30 transition-colors">
                      ...
                      <td className="p-3 align-top"><ClassBox title="Mathematics" teacher="Mr. Rahman" color="blue" /></td>
                      <td className="p-3 align-top"><ClassBox title="Physics" teacher="Dr. Karim" color="red" /></td>
                      <td className="p-3 align-top"><ClassBox title="English" teacher="Ms. Farhana" color="amber" /></td>
                      <td className="p-3 align-top"><ClassBox title="Biology" teacher="Mrs. Nasrin" color="green" /></td>
                      <td className="p-3 align-top"><ClassBox title="ICT" teacher="Mr. Jahi" color="purple" /></td>
                    </tr>
                  </React.Fragment>
                ))}
              </tbody>
...
/* --- Refined ClassBox with Teacher Names --- */
const ClassBox = ({ title, teacher, color }) => {
  const colors = {
  ...
  return (
    <div className={`p-3 rounded-2xl border ${colors[color] || colors.blue} shadow-sm hover:shadow-md transition-all cursor-pointer`}>
      <p className="font-black text-xs leading-tight mb-1">{title}</p>
      <p className="text-[10px] font-bold opacity-60 flex items-center gap-1 italic">
        {teacher}
      </p>
    </div>
  );
...
export default PrincipalClassRoutine;