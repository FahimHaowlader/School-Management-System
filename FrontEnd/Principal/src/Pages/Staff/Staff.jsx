...
const Icon = ({ name, className = "" }) => {
  const icons = {
    ...
    shield: <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.57-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.286z" />
  };
  return (
...
const FacultyRow = ({ status, name, academicEmail, facultyId, coreFocusDomains, leadershipRank, imageUrl }) => {
  const badgeStyles = {
    "Active Deployment": "bg-emerald-50 text-emerald-700 border-emerald-100",
    "On Sabbatical": "bg-amber-50 text-amber-700 border-amber-100",
  ...
  return (
    <tr className="border-b border-slate-200/70 hover:bg-slate-50/70 transition-colors">
      ...
      {/* Faculty Profile and Institutional Contact */}
      <td className="p-4 align-middle">
        <div className="flex items-center gap-3 text-left">
          <div className="w-10 h-10 shrink-0 overflow-hidden rounded-full border border-slate-200 bg-slate-50">
            <img src={imageUrl} alt={name} className="w-full h-full object-cover" />
          </div>
          <div className="flex flex-col">
            <span className="text-sm font-bold text-slate-900 tracking-tight flex items-center gap-1.5">
              {name}
              {(leadershipRank.includes("VP") || leadershipRank.includes("Chair") || leadershipRank.includes("Dean")) && (
                <Icon name="shield" className="w-3.5 h-3.5 text-blue-600 stroke-[2.3]" />
              )}
            </span>
            <span className="text-xs text-slate-400 font-medium">{academicEmail}</span>
          </div>
        </div>
      </td>

      {/* Unique Board Registry ID */}
      <td className="p-4 align-middle text-left font-mono text-xs font-semibold text-slate-600">
        {facultyId}
      </td>

      {/* Curriculum Mandates & Departments */}
      <td className="p-4 align-middle text-left">
        <div className="flex flex-wrap gap-1.5 max-w-xs">
          {coreFocusDomains.map((domain, i) => (
      ...
      <td className="p-4 align-middle text-right">
        <div className="flex items-center justify-end gap-1.5">
          <button 
            onClick={() => console.log("Modify Faculty Param Clicked")}
          ...
          <button 
            onClick={() => console.log("Archive/Delete Profile Clicked")}
...
const Staff = () => {
  const leadershipRenders = ["All Leadership", "Cabinet Executive", "Department Chairs", "Specialist Faculty", "Administrative Operations"];
  const [isComposerOpen, setIsComposerOpen] = useState(false);

  // --- CORE INSTITUTIONAL FACULTY & BOARD MATRIX ---
  const [facultyDirectory, setFacultyDirectory] = useState([
    {
      id: 701,
      status: "Active Deployment",
      name: "Dr. Evelyn Thorne",
      academicEmail: "e.thorne@centralnet.edu",
      facultyId: "FAC-2026-0012",
      coreFocusDomains: ["STEM Curricula", "Academic Standards Board"],
      leadershipRank: "Vice Principal",
    ...
    },
    {
      ...
      academicEmail: "m.vance@centralnet.edu",
      facultyId: "FAC-2026-0102",
      coreFocusDomains: ["Student Life Protocols", "Disciplinary Advisory"],
      leadershipRank: "Dean of Students",
      imageUrl: "https://images.unsplash.com/photo-1511895426328-dc8714191300?auto=format&fit=crop&q=80&w=150"
    },
    {
      id: 703,
      status: "On Sabbatical",
      name: "Prof. Arthur Jenkins",
      academicEmail: "a.jenkins@centralnet.edu",
      facultyId: "FAC-2026-0891",
      coreFocusDomains: ["Fiscal Budget Audits", "Alumni Endowments"],
      leadershipRank: "Department Chair",
      imageUrl: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&q=80&w=150"
    }
  ]);

  // --- EXECUTIVE REVISIONS SUBMITTED FOR THE PRINCIPAL ---
  const [pendingEdits, setPendingEdits] = useState([
    {
      id: 881,
      targetFacultyId: 702, // Corresponds to Marcus Vance
      name: "Marcus Vance, M.Ed.",
      requestedBy: "Registrar Sterling",
      timestamp: "12 mins ago",
      oldValue: { leadershipRank: "Dean of Students", coreFocusDomains: "Student Life Protocols" },
      newValue: { leadershipRank: "Interim Vice Principal", coreFocusDomains: "Academic Standards Board" }
    }
  ]);

  // --- EXECUTIVE ACTION HANDLERS FOR THE PRINCIPAL ---
  const handleApproveEdit = (editId, targetId, updatedValues) => {
    setFacultyDirectory(facultyDirectory.map(faculty => 
      faculty.id === targetId 
        ? { ...faculty, leadershipRank: updatedValues.leadershipRank, coreFocusDomains: [updatedValues.coreFocusDomains] }
        : faculty
    ));
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200/60 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="text-left">
            <h2 className="text-sm font-bold text-slate-900 tracking-tight">Principal's Operational & Faculty Command Matrix</h2>
            <p className="text-xs text-slate-400 font-medium mt-0.5">Regulate institutional cabinet structures, assign instructional oversight jurisdictions, and audit administrative policies.</p>
          </div>
          
          <div className="flex items-center gap-4 border-l border-transparent sm:border-slate-100 sm:pl-4">
            <div className="text-center sm:text-right">
              <span className="block text-xl font-black text-slate-900 leading-none">1,482</span>
              <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Total Enrollment Registry</span>
            </div>
          </div>
        </div>

        {/* --- EXECUTIVE OVERRIDE AUTHORIZATION QUEUE PANEL (Matches Parent Structure) --- */}
                ...
                <div key={edit.id} className="p-5 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 bg-white/50">
                  <div className="space-y-1 text-left">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-[10px] bg-slate-200 text-slate-700 px-2 py-0.5 rounded font-bold uppercase tracking-tight">Profile Adjustment</span>
                    </div>
                  ...
                  <div className="flex items-center gap-4 text-xs">
                    <div className="bg-slate-100/80 border border-slate-200/60 rounded-lg px-3 py-1.5 text-slate-500 text-left">
                      ...
                      <span className="font-medium">{edit.oldValue.leadershipRank} • {edit.oldValue.coreFocusDomains}</span>
                    </div>
                    ...
                    <div className="bg-amber-100/80 border border-amber-200/80 rounded-lg px-3 py-1.5 text-amber-900 text-left">
                      ...
                      <span className="font-bold">{edit.newValue.leadershipRank} • {edit.newValue.coreFocusDomains}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 w-full lg:w-auto self-end lg:self-center justify-end">
                    ...
                    <button 
                      onClick={() => handleApproveEdit(edit.id, edit.targetFacultyId, edit.newValue)}
        ...
        {/* Board Appointed Onboarding Form */}
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm overflow-hidden transition-all">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-bold text-slate-800 tracking-wide">Register New Leadership Board Commission</span>
            </div>
            ...
            <div className="p-5 border-t border-slate-100 space-y-4 bg-white text-left animate-fadeIn">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Active Contract Status</label>
                ...
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Oversight Jurisdictions / Subjects</label>
                  <input 
                    type="text" 
                    placeholder="e.g., Department of Fine Arts, STEM Curricula" 
                ...
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Academic Rank Tier</label>
                  <select className="w-full h-10 px-3 border border-slate-200 rounded-lg text-xs font-semibold bg-white outline-none focus:border-slate-900 transition-colors">
                    <option>Vice Principal</option>
                    <option>Dean of Students</option>
                    <option>Department Chair</option>
                    <option>Specialist Faculty</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Faculty Legal Full Name & Credentials</label>
                  <input 
                    type="text" 
                    placeholder="e.g., Prof. Sarah Jenkins, PhD" 
                    className="w-full h-10 px-3 border border-slate-200 rounded-lg text-xs outline-none focus:border-slate-900 transition-colors"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Institutional Academic Email Node</label>
                  <input 
                    type="email" 
                    placeholder="e.g., s.jenkins@centralnet.edu" 
                    className="w-full h-10 px-3 border border-slate-200 rounded-lg text-xs outline-none focus:border-slate-900 transition-colors"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Board Registry Identity String</label>
                  <input 
                    type="text" 
                    placeholder="e.g., FAC-2026-0891" 
                    className="w-full h-10 px-3 border border-slate-200 rounded-lg text-xs outline-none focus:border-slate-900 transition-colors"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] uppercase font-bold text-slate-500 tracking-wider">Official Database Passport/Avatar File</label>
                <div className="border border-dashed border-slate-200 rounded-xl p-5 bg-slate-50/50 hover:bg-slate-50 transition-colors flex flex-col items-center justify-center gap-1.5 group/dropzone cursor-pointer relative">
                  ...
                  <p className="text-xs font-bold text-slate-700 mt-0.5">Upload a clean institutional face verification picture</p>
        ...
        {/* Search Engine and Structural Rank Filters */}
        <div className="flex flex-col lg:flex-row items-stretch gap-3">
          <div className="relative flex-grow h-11 group">
            <input 
              type="text" 
              placeholder="Search personnel database by educator profiles, specific departments, registry IDs, or core domains..." 
        ...
        {/* Administrative Registry Data Grid */}
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200/80 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                  <th className="p-4">Appointed Officer Profile</th>
                  <th className="p-4">Board Registry ID</th>
                  <th className="p-4">Oversight Jurisdictions</th>
                  <th className="p-4 hidden md:table-cell">Institutional Rank Tier</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {facultyDirectory.map((faculty) => (
                  <FacultyRow 
                    ...
                    name={faculty.name}
                    academicEmail={faculty.academicEmail}
                    facultyId={faculty.facultyId}
                    coreFocusDomains={faculty.coreFocusDomains}
                    leadershipRank={faculty.leadershipRank}
