...
const ParentRow = ({ status, name, emailAddress, phoneNumber, linkedStudents, relationRole, imageUrl }) => {
  ...
  return (
    <tr className="border-b border-slate-200/70 hover:bg-slate-50/70 transition-colors">
      ...
      {/* Linked Student Network - Formatted in an organized column style */}
...
const Parent = () => {
  ...
  // --- REVIEWS / PENDING HOUSEHOLD EDITS SUBMITTED BY OTHER STAFF ---
  const [pendingEdits, setPendingEdits] = useState([
    {
      id: 881,
      targetParentId: 102, // Corresponds to Helena Rostova
      name: "Helena Rostova",
      requestedBy: "Registrar Sterling",
      timestamp: "12 mins ago",
      oldValue: { relationRole: "Mother", phoneNumber: "(555) 014-2900" },
      newValue: { relationRole: "Legal Guardian", phoneNumber: "(555) 014-9988" }
    }
  ]);

  // --- ACTION HANDLERS FOR STAFF EDITS ---
  const handleApproveEdit = (editId, targetId, updatedValues) => {
    setParentDirectory(parentDirectory.map(parent => 
      parent.id === targetId 
        ? { ...parent, relationRole: updatedValues.relationRole, phoneNumber: updatedValues.phoneNumber }
        : parent
    ));
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200/60 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 border-l border-transparent sm:border-slate-100 sm:pl-4">
            <div className="text-center sm:text-right">
              <span className="block text-xl font-black text-slate-900 leading-none">{parentDirectory.length + 1399}</span>
                ...
                <div key={edit.id} className="p-5 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 bg-white/50">
                  ...
                  <div className="flex items-center gap-4 text-xs">
                    <div className="bg-slate-100/80 border border-slate-200/60 rounded-lg px-3 py-1.5 text-slate-500 text-left">
                      ...
                      <span className="font-medium">{edit.oldValue.relationRole} • {edit.oldValue.phoneNumber}</span>
                    </div>
                    ...
                    <div className="bg-amber-100/80 border border-amber-200/80 rounded-lg px-3 py-1.5 text-amber-900 text-left">
                      ...
                      <span className="font-bold">{edit.newValue.relationRole} • {edit.newValue.phoneNumber}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 w-full lg:w-auto self-end lg:self-center justify-end">
                    ...
                    <button 
                      onClick={() => handleApproveEdit(edit.id, edit.targetParentId, edit.newValue)}
              ...
              <tbody className="divide-y divide-slate-100">
                {parentDirectory.map((parent) => (
                  <ParentRow 
                    ...
                    relationRole={parent.relationRole}
