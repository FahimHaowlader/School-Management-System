    ...
    <tr className="border-b border-slate-200/70 hover:bg-slate-50/70 transition-colors">
      ...
      <td className="p-4 align-middle text-left">
        <div className="flex flex-wrap gap-1.5 max-w-xs">
          {assignedSubjects.map((subject, i) => (
            <span key={i} className="inline-flex items-center gap-1 text-[11px] font-semibold bg-blue-50 border border-blue-100 text-blue-700 px-2.5 py-0.5 rounded-md whitespace-nowrap">
              {subject}
            </span>
          ))}
        </div>
      ...
      <td className="p-4 align-middle text-right">
        <div className="flex items-center justify-end gap-1.5">
          <button 
            onClick={() => console.log("Principal updating faculty parameters")}
          ...
          <button 
            onClick={() => console.log("Principal executing faculty separation protocol")}
...
const Teacher = () => {
  ...
  // --- FACULTY REVISIONS SUBMITTED FOR PRINCIPAL EXECUTIONS ---
  const [pendingEdits, setPendingEdits] = useState([
    {
      id: 951,
      targetFacultyId: 102, // Maps to Prof. Sarah Connor
      name: "Prof. Sarah Connor",
      requestedBy: "Academic Registrar",
      timestamp: "8 mins ago",
      oldValue: { department: "Science", assignedSubjects: "Physics (9-A)" },
      newValue: { department: "Science Research", assignedSubjects: "Advanced Quantum Lab" }
    }
  ]);

  // --- PRINCIPAL AUTHORIZATION ACTION HANDLERS ---
  const handleApproveEdit = (editId, targetId, updatedValues) => {
    setFacultyDirectory(facultyDirectory.map(faculty => 
      faculty.id === targetId 
        ? { ...faculty, department: updatedValues.department, assignedSubjects: [updatedValues.assignedSubjects] }
        : faculty
    ));
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200/60 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="text-left">
            <h2 className="text-sm font-bold text-slate-900 tracking-tight">Faculty & Teacher Command Matrix</h2>
            <p className="text-xs text-slate-400 font-medium mt-0.5">Manage institutional teacher records, map core departments, track deployment statuses, and audit class matrices.</p>
          </div>
        ...
        {/* --- AWAITING EXECUTIVE AUTHORIZATION QUEUE PANEL (Parent Style Layout) --- */}
                ...
                <div key={edit.id} className="p-5 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 bg-white/50">
                  ...
                  <div className="flex items-center gap-4 text-xs">
                    <div className="bg-slate-100/80 border border-slate-200/60 rounded-lg px-3 py-1.5 text-slate-500 text-left">
                      ...
                      <span className="font-medium">{edit.oldValue.department} • {edit.oldValue.assignedSubjects}</span>
                    </div>
                    ...
                    <div className="bg-amber-100/80 border border-amber-200/80 rounded-lg px-3 py-1.5 text-amber-900 text-left">
                      ...
                      <span className="font-bold">{edit.newValue.department} • {edit.newValue.assignedSubjects}</span>
                    </div>
                  </div>
